...
def parse_f1(f, dicband):
    f1score = {}
    band = []
    for i in f:
        if "Number of bands" in i:
            band = i.split(":")[-1][1:-1]
            band = convert_str_to_list(band)
            # print(find_band(dicband, band), band)
            f1tot = []
        ...
        if find_band(dicband, band) != None:
            f1score[find_band(dicband, band)] = np.array(f1tot)
    return f1score


def plot_boxplot_channel_aux(f1score):
